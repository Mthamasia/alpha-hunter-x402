...
import { EXPECTED, MAINNET_GENESIS_HASH } from "./config";
...
function networkLabel(network: string): string {
  if (network === EXPECTED.network) return "Algorand Testnet";
  if (network.includes(MAINNET_GENESIS_HASH)) return "Algorand MAINNET (bloqueada)";
  return "desconhecida";
}
 ...
 * Exige exatamente UMA opção de pagamento, e ela precisa bater em tudo.
 */
export function validatePaymentRequired(pr: PaymentRequired, payer: string): ValidationResult {
  ...
  if (req.network !== EXPECTED.network) errors.push(`network "${req.network}" não é Algorand Testnet`);
  if (req.network.includes(MAINNET_GENESIS_HASH)) errors.push("network é MAINNET — bloqueado");
  ...
  if (!payer) errors.push("nenhuma carteira conectada (payer vazio)");
  if (payer && payer === req.payTo) errors.push("payer e payTo são o mesmo endereço");
