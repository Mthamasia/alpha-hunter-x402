...
import { EXPECTED, INVESTIGATE_PATH, PAY_BUTTON_LABEL, PERA_TESTNET_CHAIN_ID } from "./config";
...
// Pera travada na TestNet (chainId 416002). A chave nunca sai da Pera.
const pera = new PeraWalletConnect({ chainId: PERA_TESTNET_CHAIN_ID, shouldShowSignTxnToast: true });
...
function renderSummary(s: PaymentSummary | null) {
  ...
  if (!s) return;
  ui.summary.append(
    row("Network", `${s.networkLabel} (${s.network})`, `Algorand Testnet (${EXPECTED.network})`),
...
function renderPay(o: PayOutcome) {
  ...
  if (o.transactionId) {
    ...
    a.href = `https://lora.algokit.io/testnet/transaction/${encodeURIComponent(o.transactionId)}`;
    a.target = "_blank";
...
ui.quote.addEventListener("click", async () => {
  ...
  switch (q.kind) {
    ...
    case "no-payment-required":
      ui.quoteStatus.textContent = `HTTP ${q.status} sem cobrança (servidor não está em x402-testnet?).`;
