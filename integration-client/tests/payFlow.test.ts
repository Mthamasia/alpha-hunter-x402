...
import { PAYER, TYPO_PAY_TO, header402, paymentRequired, requirement, response } from "./fixtures";
...
describe("fluxo manual x402", () => {
  ...
  it("pagamento completo: 1 assinatura, 1 reenvio com PAYMENT-SIGNATURE, settlement exibido", async () => {
    ...
    expect(scheme.calls[0]).toMatchObject({ amount: "50000", asset: "10458941", payTo: EXPECTED.payTo });
  ...
  it("402 divergente (typo no payTo) -> recusado; pagar é impossível", async () => {
    const bad = header402(paymentRequired([requirement({ payTo: TYPO_PAY_TO })]));
