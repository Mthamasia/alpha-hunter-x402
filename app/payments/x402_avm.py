"""x402 V2 real em Algorand Testnet/Mainnet.
        ...
        INVESTIGATE_ROUTE: {
            **(
            ...
            "accepts": {
                ...
                # AssetAmount explícito em unidades mínimas para não depender
                # de conversão via float no parser padrão de preço.
                #
                # A tag do Global x402 Challenge precisa fazer parte do
                # PaymentRequirements enviado ao facilitator no settlement.
                "price": {
...
def build_x402_middleware(settings: Settings, facilitator_client: Any = None):
    """Retorna a middleware FastAPI oficial do x402 para Algorand.

    `facilitator_client` permite injetar um facilitator falso nos testes.
    """
    server = x402ResourceServer(
        facilitator_client or build_facilitator_client(settings)
    )
