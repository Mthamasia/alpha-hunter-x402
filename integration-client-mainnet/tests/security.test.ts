...
describe("pagamento só com ação explícita / somente Mainnet", () => {
  ...
  it("Pera travada em Mainnet (chainId 416001)", () => {
    expect(read(join(SRC, "config.ts"))).toMatch(/PERA_MAINNET_CHAIN_ID = 416001/);
    expect(main).toMatch(/new PeraWalletConnect\(\{ chainId: PERA_MAINNET_CHAIN_ID/);
    expect(main).not.toMatch(/416002/);
  });

  it("botão PAY 0.05 USDC MAINNET existe e começa desabilitado", () => {
    const html = read(join(ROOT, "index.html"));
    expect(html).toMatch(/<button id="pay" disabled>PAY 0\.05 USDC MAINNET<\/button>/);
    expect(read(join(SRC, "config.ts"))).toMatch(/PAY_BUTTON_LABEL = "PAY 0\.05 USDC MAINNET"/);
  });
