 ...
 * assinar é exatamente a transferência esperada (Mainnet, ASA, amount, payTo).
 */
...
export interface RoundGuard {
  /** Round atual da Mainnet (algorand.network.getLastRound()). */
...
export function assertExpectedPaymentTxn(txn: algosdk.Transaction, payer: string): void {
  ...
  if (payer !== EXPECTED.payer) problems.push("payer não é a carteira Mainnet autorizada");
  if (txn.sender.toString() !== payer) problems.push("sender ≠ carteira conectada");
  if (txn.genesisID !== EXPECTED.genesisId) problems.push(`genesisID ${txn.genesisID} não é Mainnet`);
  if (!txn.genesisHash || b64(txn.genesisHash) !== EXPECTED.genesisHash) {
    problems.push("genesisHash não é Mainnet");
  }
...
export function createPeraSigner(
  ...
  if (address !== EXPECTED.payer) throw new Error("Endereço Pera não é o payer Mainnet autorizado.");
  let used = false;
  ...
  return {
    ...
    async signTransactions(txns: Uint8Array[], indexesToSign?: number[]) {
      ...
      if (decoded.length !== 2) throw new Error(`Grupo x402 inesperado: ${decoded.length} transações.`);
      if (toSign.length !== 1 || toSign[0] !== 1) {
        throw new Error("Esperada somente a transferência USDC do payer no índice 1.");
      }
      const feeTxn = decoded[0]!;
      if (
        feeTxn.type !== algosdk.TransactionType.pay ||
        feeTxn.sender.toString() !== EXPECTED.feePayer ||
        feeTxn.payment?.receiver.toString() !== EXPECTED.feePayer ||
        feeTxn.payment.amount !== 0n ||
        feeTxn.rekeyTo
      ) {
        throw new Error("Transação de fee payer inesperada.");
      }
      ...
      for (const txn of decoded) {
        if (txn.genesisID !== EXPECTED.genesisId) throw new Error("Grupo contém transação fora da Mainnet.");
      }
